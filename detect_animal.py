...
ANIMAL_WEIGHTS_PATH = r'D:/road_safety/models/animal_sign_yolov8.pt'
YOLO_INPUT_SIZE = 640
CONF_THRESHOLD = 0.7
...
RECIPIENT_EMAIL = 'dipak6541.6615@gmail.com'
...
def send_alert_email(detected_animals, lat, lng, formatted_address):
    """Send email alert with detected animal names and live location coordinates."""
    msg = MIMEMultipart()
    msg['From'] = AUTH_EMAIL
    msg['To'] = RECIPIENT_EMAIL
    msg['Subject'] = 'Road Safety Alert: Animal Rush Detected'
    ...
    body = f"""
    An animal rush has been detected on the road.

    Details:
    - Detected Animals: {', '.join(detected_animals) if detected_animals else 'None'}
    ...
    try:
        ...
        server.quit()
        logging.info("Alert email sent successfully for animal rush")
    except Exception as e:
...
def select_video_file_gui():
    ...
    tk.Label(root, text="Select Video for Animal Detection", font=("Arial", 16)).pack(pady=20)
...
def draw_detection_boxes(frame, detections, class_names, color=(0, 255, 0)):
...
def process_video(video_path, model, class_names):
    """Process video for animal detection, display results, and send alerts."""
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        logging.error(f"Cannot open video: {video_path}")
        return
    win_name = "Animal Detection"
    ...
    while True:
        ...
        for r in results:
            for box in r.boxes:
                ...
                if class_name.lower() not in ['unknown', 'potholes', 'landslide'] and conf >= CONF_THRESHOLD:
                    x1, y1, x2, y2 = int(x1 * scale_x), int(y1 * scale_y), int(x2 * scale_x), int(y2 * scale_y)
                    detections.append({'box': [x1, y1, x2, y2], 'confidence': conf, 'class_id': cls})
                    frame_animals.add(class_names[cls])
        
        if frame_animals:
            alert_message = "Animal detected, slow down!"
            detected_animals.update(frame_animals)
            if not email_sent:
                send_alert_email(detected_animals, lat, lng, formatted_address)
                email_sent = True
        else:
            alert_message = ""
            email_sent = False
        ...
        draw_detection_boxes(video_disp, detections, list(class_names.values()), color=(0, 255, 0))
        ...
        y = 60
        cv2.putText(data_panel, f"Animals: {len(detections)}", (30, y), font, 1.0, (0,200,0) if len(detections)<=8 else (0,0,255), 2)
        y += 40
...
def main():
    """Main function to run animal detection."""
    test_connectivity()
    model = YOLO(ANIMAL_WEIGHTS_PATH)
