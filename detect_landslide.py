...
LANDSLIDE_WEIGHTS_PATH = r'D:/road_safety/models/landslide_yolov8.pt'
...
RECIPIENT_EMAIL = 'prateekkumawat18@gmail.com'
...
def send_alert_email(confidence, lat, lng, formatted_address):
    """Send email alert with landslide detection and live location coordinates."""
    msg = MIMEMultipart()
    msg['From'] = AUTH_EMAIL
    msg['To'] = RECIPIENT_EMAIL
    msg['Subject'] = 'Road Safety Alert: Landslide Detected'
    ...
    body = f"""
    A landslide has been detected on the road.
    ...
    try:
        ...
        server.quit()
        logging.info("Alert email sent successfully for landslide")
    except Exception as e:
...
def select_video_file_gui():
    ...
    tk.Label(root, text="Select Video for Landslide Detection", font=("Arial", 16)).pack(pady=20)
...
def draw_detection_boxes(frame, detections, class_names, color=(0, 0, 255)):
...
def process_video(video_path, model, class_names):
    """Process video for landslide detection, display results, and send alerts."""
    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        logging.error(f"Cannot open video: {video_path}")
        return
    win_name = "Landslide Detection"
    ...
    while True:
        ...
        for r in results:
            for box in r.boxes:
                ...
                if class_name.strip().lower() == 'landslide' and conf > CONF_THRESHOLD:
                    x1, y1, x2, y2 = int(x1 * scale_x), int(y1 * scale_y), int(x2 * scale_x), int(y2 * scale_y)
                    det = {'box': [x1, y1, x2, y2], 'confidence': conf, 'class_id': cls}
                    if best_det is None or conf > best_det['confidence']:
                        best_det = det
        detections = [best_det] if best_det else []
        logging.info(f"Detected classes this frame (landslide): {detected_classes}")
        
        if detections:
            alert_message = "Landslide ahead, stop!"
            if not email_sent:
                send_alert_email(detections[0]['confidence'], lat, lng, formatted_address)
                email_sent = True
        else:
            alert_message = ""
            email_sent = False
        ...
        draw_detection_boxes(video_disp, detections, list(class_names.values()), color=(0, 0, 255))
        ...
        y = 60
        cv2.putText(data_panel, f"Landslides: {len(detections)}", (30, y), font, 1.0, (0,200,0) if len(detections)==0 else (0,0,255), 2)
        y += 40
...
def main():
    """Main function to run landslide detection."""
    test_connectivity()
    model = YOLO(LANDSLIDE_WEIGHTS_PATH)
